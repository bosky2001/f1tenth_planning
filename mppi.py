...
#     def rollout_step(self, state, a, ref_state):
#         """
#         function to do 1 rollout given actions and state
#         """
        
#         #  do the whole rollout with n actions?
#         # a = jnp.reshape(a, env.a_shape)
#         a, delta = a[0].item(), a[1].item()
#         state = self.update_state_kinematic(state, a, delta)
#         cost  = self.stage_cost(state, ref_state)
        
#         # r = env.reward(env_state)
#         return state, cost
    
#     def rollouts(self, state, actions):
